...
if __name__ == "__main__":
    ...
    configs = BaseModelConfigs.load("Models/05_sound_to_text/202503171609/configs.yaml")

    model = WavToTextModel(model_path=configs.model_path, char_list=configs.vocab, force_cpu=False)

    # df = pd.read_csv("Models/05_sound_to_text/202503171609/val.csv").values.tolist()
    
    data = {
        "wav_path": ["../audio0.wav"],
        "label": ["concord returned to its place amongs the tents"]
    }
    ...
    for wav_path, label in df.values:
        ...
        print(f"True: {true_label}, Predicted: {text}, CER: {cer}, WER: {wer}")
