...
# Define these outside of any function to make them importable in child processes
def process_row(row_dict, mfcc_transform, mfcc_transform_func, audio_bytes_to_ndarray_func):
    """Process a single row of data."""
    # Apply MFCC transform
    mfccs_untransposed = mfcc_transform_func(mfcc_transform, torch.tensor(np.array(audio_bytes_to_ndarray_func(row_dict['audio']['bytes']))).float())
    
    return [
        mfccs_untransposed,
        mfccs_untransposed.transpose(0, 1),
        row_dict['text'],
    ]

# Create a wrapper function for your mfcc_transform
def mfcc_transform_wrapper(mfcc_transform, audio_tensor):
    return mfcc_transform(audio_tensor)

# Create a wrapper function for audio conversion
def audio_bytes_to_ndarray_wrapper(audio_bytes):
    return parquet_dataframe.audio_bytes_to_ndarray(audio_bytes)
...
if __name__ == '__main__':
    ...
    for filename in files_source:
        ...
        dataframe = parquet_dataframe.dataframe_from_parquet(filepath)
        print(f"Dataframe shape: {dataframe.shape}")
        
        # Convert each row to a dictionary with only the necessary data
        rows_to_process = []
        ...
        # Use wrapper functions instead of passing module functions directly
        process_func = partial(
            process_row, 
            mfcc_transform=mfcc_transform,
            mfcc_transform_func=mfcc_transform_wrapper,
            audio_bytes_to_ndarray_func=audio_bytes_to_ndarray_wrapper
        )
        
        # Process rows in parallel
        with mp.Pool(processes=mp.cpu_count() // 2) as pool:
            results = pool.map(process_func, rows_to_process)
