...
# from torchaudio.transforms import MFCC # Removed MFCC import
...
class ASRDataset(Dataset):
    def __init__(self, dataset, tokenizer): # Removed mfcc_transform
        self.dataset = dataset
        # self.mfcc_transform = mfcc_transform # Removed
    ...
    def __getitem__(self, idx):
        ...
        # mfcc = self.mfcc_transform(spectrogram) # Removed
        tokens = self.tokenizer(transcript)
        return spectrogram.T, torch.tensor(tokens), spectrogram # Returned original spectrogram

def collate_fn_asr(batch):
    spectrograms_transposed_batch = [torch.tensor(item[0]).to(device).float() for item in batch] # These are now transposed spectrograms
    tokens_batch = [item[1] for item in batch]
    spectrograms_untransposed_batch = [item[2] for item in batch] # Get untransposed spectrograms

    spectrograms_padded = pad_sequence(spectrograms_transposed_batch, batch_first=True)
    tokens_padded = pad_sequence(tokens_batch, batch_first=True, padding_value=0)
    input_lengths = torch.tensor([spectrogram.shape[1] for spectrogram in spectrograms_untransposed_batch]) # Use untransposed for length (shape[1] is time_frames)
    target_lengths = torch.tensor([len(tokens) for tokens in tokens_batch])

    return spectrograms_padded, tokens_padded, input_lengths, target_lengths
...
for filename in files_source:
    ...
    # dataframe = dataframe.head(200) # Load a subset for testing
...
train_asr_dataset = ASRDataset(train_dataset, tokenize_text) # Removed mfcc_transform
dev_asr_dataset = ASRDataset(dev_dataset, tokenize_text) # Removed mfcc_transform
...
class MediumASR(nn.Module):
    def __init__(self, input_dim, output_dim, dropout=0.2):
        ...
        # RNN Layers
        self.lstm1 = nn.LSTM(1568, 128, bidirectional=True, batch_first=True)
    ...
    def forward(self, x):
        ...
        # RNN Layers
        x, _ = self.lstm1(x)
        # print("After lstm1 x shape:", x.shape)
...
# n_mfcc = 40 # Removed as not used
# input_dim = n_mfcc # Changed below
output_dim = len(characters)
# The input dimension should be the frequency dimension of the spectrogram
# From custom_spectrogram: fft_length = 384, so frequency bins = fft_length // 2 + 1 = 193
input_dim = 193
...
def main():
    if TRAIN:
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (spectrograms_padded, tokens_padded, input_lengths, target_lengths) in enumerate(train_dataloader):
                spectrograms_padded = spectrograms_padded.to(device)
                tokens_padded = tokens_padded.to(device)
                input_lengths = input_lengths.to(device)
                target_lengths = target_lengths.to(device)

                optimizer.zero_grad()
                # log_probs = model(spectrograms_padded, input_lengths) # Removed input_lengths from forward
                log_probs = model(spectrograms_padded)
                ...
                if batch_idx % batch_interval == 0 and batch_idx > 0:
                    ...
                    # torch.save(model.state_dict(), "asr_v5.pth")
            ...
            dev_loss = 0.0
            with torch.no_grad():
                for batch_idx, (spectrograms_padded, tokens_padded, input_lengths, target_lengths) in enumerate(dev_dataloader):
                    spectrograms_padded = spectrograms_padded.to(device)
                    tokens_padded = tokens_padded.to(device)
                    input_lengths = input_lengths.to(device)
                    target_lengths = target_lengths.to(device)
        ...
        model.load_state_dict(torch.load("asr_v5.pth"))
        model.eval()
    ...
    if True:
        with torch.no_grad():
            for batch_idx, (spectrograms_padded, tokens_padded, input_lengths, target_lengths) in enumerate(dev_dataloader):
                ...
                batch_wer = calculate_wer(reference_sentences, predicted_sentences[0]) # Taking the first prediction from multiple
