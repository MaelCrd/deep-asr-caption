...
if __name__ == '__main__':
    ...
    def ctc_decoder(log_probs, input_lengths, raw=False):
        ...
        for i in range(predicted_tokens.shape[0]):
            ...
            if not raw: tokens = [k for k, g in groupby(tokens)]
            sentence = ""
            for t in tokens:
            ...
            for epoch in range(num_epochs):
                ...
                for batch_idx, (mfccs_padded, tokens_padded, input_lengths, target_lengths) in enumerate(train_dataloader):
                    ...
                    if batch_idx % batch_interval == 0 and batch_idx > 0:
                        ...
                        print(f"[{datetime.now().strftime("%d-%m %H:%M")}] Epoch [{epoch+1}/{num_epochs}], Batch [{batch_idx}/{len(train_dataloader)}]", end="\r")
